...
def robots_txt():
    # Crawling blocked. The X-Robots-Tag: noindex, nofollow header shipped
    # first (PR #40), and this Disallow is merged only once Search Console
    # confirms the backend URLs have dropped out of the index (date in the PR).
...
def _run_free_build(report_id, inputs):
    ...
    try:
        ...
        anchor_bias = None
        if _be and not buyer_estimate_implausible and report.get("weighted_midpoint"):
            try:
